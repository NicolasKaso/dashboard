...
function renderDeadlines() {
  ...
  const now  = new Date(); now.setHours(0, 0, 0, 0);
  const src = isTokenValid() ? gcalDeadlines : deadlines;
  if (!src.length) { list.innerHTML = '<div class="empty-state">No upcoming deadlines</div>'; return; }
...
function deleteDeadline(id) {
  ...
  if (isTokenValid()) {
    const dl = gcalDeadlines.find(d => String(d.id) === String(id));
    if (dl && dl.gcalEventId) {
      fetch(
...
function confirmDeadline() {
  ...
  // If connected, Calendar is source of truth: create event and refresh list
  if (isTokenValid()) {
