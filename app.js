...
const CalSlider = {
  ...
  // Fresh mount — no animation. Builds prev/cur/next panels.
  mount(container, buildFn, centerDate, view) {
    const [p, c, n] = this._neighbors(centerDate, view);
    container.innerHTML =
      '<div class="cal-slider-viewport">' +
        '<div class="cal-slider-track" id="cal-slider-track">' +
          '<div class="cal-slider-panel">' + buildFn(p) + '</div>' +
          '<div class="cal-slider-panel">' + buildFn(c) + '</div>' +
          '<div class="cal-slider-panel">' + buildFn(n) + '</div>' +
        '</div>' +
      '</div>';
    this._applyScroll(container, this._savedScroll);
  },

  // Animated slide then rebuild.
  // dir: 1 = forward (slide left), -1 = backward (slide right)
  slide(container, buildFn, newCenterDate, view, dir) {
    if (this._animating) return;
    const track = document.getElementById('cal-slider-track');
    if (!track) { this.mount(container, buildFn, newCenterDate, view); return; }

    // Snapshot scroll before anything changes
    const scrollEl = container.querySelector('.tg-scroll');
    if (scrollEl) this._savedScroll = scrollEl.scrollTop;

    this._animating = true;

    // Rebuild all panels with the new neighbor dates
    const [p, c, n] = this._neighbors(newCenterDate, view);
    const panels = track.querySelectorAll('.cal-slider-panel');
    if (panels.length === 3) {
      panels[0].innerHTML = buildFn(p);
      panels[1].innerHTML = buildFn(c);
      panels[2].innerHTML = buildFn(n);
    }

    // Restore scroll on all panels immediately so animation starts right
    this._applyScroll(container, this._savedScroll);

    // The track currently shows center (-100%). We animate to show what
    // looks like the adjacent panel, then reset to center.
    // Because we've already rebuilt panels, after the reset the user sees new center.
    const from = -100;
    const to = dir === 1 ? -200 : 0; // slide left → next, slide right → prev

    requestAnimationFrame(() => {
      track.style.transition = 'none';
      track.style.transform = 'translateX(' + from + '%)';
      // Force reflow
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(() => { if (this._animating) done(); }, 500);
      });
    });
  },

  // Refresh panels in-place after events load — no animation, no scroll jump
  refresh(container, buildFn, centerDate, view) {
    const track = document.getElementById('cal-slider-track');
    if (!track) return;
    const [p, c, n] = this._neighbors(centerDate, view);
    const dates = [p, c, n];
    ...
    panels.forEach((panel, i) => {
      const scrollEl = panel.querySelector('.tg-scroll');
      const sc = scrollEl ? scrollEl.scrollTop : 0;
      panel.innerHTML = buildFn(dates[i]);
      const ns = panel.querySelector('.tg-scroll');
      if (ns) ns.scrollTop = sc;
    });
  },

  _applyScroll(container, top) {
    requestAnimationFrame(() => {
      container.querySelectorAll('.tg-scroll').forEach(el => { el.scrollTop = top; });
    });
  },

  // Returns [prevDate, curDate, nextDate] as independent Date objects
  _neighbors(date, view) {
    const prev = new Date(date), next = new Date(date);
    if (view === 'day')   { prev.setDate(date.getDate()-1);           next.setDate(date.getDate()+1); }
    else if (view === 'week')  { prev.setDate(date.getDate()-7);      next.setDate(date.getDate()+7); }
    else if (view === 'month') { prev.setMonth(date.getMonth()-1);    next.setMonth(date.getMonth()+1); }
    else                       { prev.setFullYear(date.getFullYear()-1); next.setFullYear(date.getFullYear()+1); }
    return [prev, new Date(date), next];
  }
};
...
const Calendar = {
  render() {
    ...
    CalSlider.mount(container, (d) => this._buildView(d, calView), calDate, calView);
  ...
  navigate(dir) {
    ...
    CalSlider.slide(document.getElementById('cal-container'), (d) => this._buildView(d, calView), newDate, calView, dir);
    if (GoogleAPI.isTokenValid()) this.fetchEvents();
  },
  ...
  async fetchEvents() {
    ...
    CalSlider.refresh(document.getElementById('cal-container'), (d) => this._buildView(d, AppState.get().calView), AppState.get().calDate, AppState.get().calView);
  },
  ...
  _scrollToHour(hr) {
    requestAnimationFrame(() => {
      // Scroll the center panel (index 1)
      const viewport = document.getElementById('cal-slider-viewport');
      if (!viewport) return;
      const panels = viewport.querySelectorAll('.cal-slider-panel');
      if (panels[1]) {
        const el = panels[1].querySelector('.tg-scroll');
...
const Tasks = {
  render() {
    ...
    let filtered = state.currentFilter === 'all' ? [...state.tasks] : state.tasks.filter(t => t.type === state.currentFilter);
    if (!filtered.length) { list.innerHTML = '<div class="empty-state">Nothing here \u2014 add something above</div>'; return; }
    // Sort: urgent+active first → active → done
    filtered.sort((a, b) => {
      const aUrgent = a.urgent && !a.done, bUrgent = b.urgent && !b.done;
    ...
    list.innerHTML = filtered.map(t => {
      ...
      return '<div class="task-item' + uCls + dCls + '">' +
        ...
        '<button class="task-flag' + (t.urgent ? ' active' : '') + '" onclick="window.toggleUrgent(\'' + t.id + '\')" title="' + (t.urgent ? 'Remove urgent' : 'Mark urgent') + '">&#9873;</button>' +
  ...
  async add() {
    ...
    const id = Date.now();
    const tasks = [...AppState.get().tasks, { id, text, type, done: false, urgent }];
  ...
  async resolveTaskLists() {
    ...
    const post = async (title) => {
      const r = await fetch('https://tasks.googleapis.com/tasks/v1/users/@me/lists', { method: 'POST', headers: { Authorization: 'Bearer ' + AppState.get().accessToken, 'Content-Type': 'application/json' }, body: JSON.stringify({ title }) });
  ...
  async pushToGoogle(text, type, localId) {
    try {
      const ids = await this.resolveTaskLists(), listId = type === 'assignment' ? ids.assignment : ids.todo;
      const res = await fetch('https://tasks.googleapis.com/tasks/v1/lists/' + listId + '/tasks', { method: 'POST', headers: { Authorization: 'Bearer ' + AppState.get().accessToken, 'Content-Type': 'application/json' }, body: JSON.stringify({ title: text }) });
  ...
  async syncDone(task) {
    ...
    await fetch('https://tasks.googleapis.com/tasks/v1/lists/' + task.gtaskListId + '/tasks/' + task.gtaskId, { method: 'PUT', headers: { Authorization: 'Bearer ' + AppState.get().accessToken, 'Content-Type': 'application/json' }, body: JSON.stringify(updated) });
  },

  async fetchFromGoogle() {
    if (!GoogleAPI.isTokenValid()) return;
    const ids = await this.resolveTaskLists();
    const byGtaskId = {}, byTextType = {};
    AppState.get().tasks.forEach(t => { if (t.gtaskId) byGtaskId[t.gtaskId] = t; else byTextType[t.text + '||' + t.type] = t; });
    const incoming = [];
    for (const [type, listId] of [['todo', ids.todo], ['assignment', ids.assignment]]) {
      ...
      (data.items || []).forEach(t => { if (t.title) incoming.push({ gtaskId: t.id, gtaskListId: listId, text: t.title, type, done: t.status === 'completed' }); });
    }
    let tasks = AppState.get().tasks.filter(t => !t.gtaskId);
    incoming.forEach(gt => {
      const byId = byGtaskId[gt.gtaskId];
      if (byId) { tasks.push({ ...byId, done: gt.done, text: gt.text }); return; }
      const byText = byTextType[gt.text + '||' + gt.type];
      if (byText) { tasks = tasks.filter(t => t.id !== byText.id); tasks.push({ ...byText, gtaskId: gt.gtaskId, gtaskListId: gt.gtaskListId, done: gt.done }); return; }
      tasks.push({ id: 'gt_' + gt.gtaskId, gtaskId: gt.gtaskId, gtaskListId: gt.gtaskListId, text: gt.text, type: gt.type, done: gt.done, source: 'gtasks' });
    });
    AppState.set({ tasks }); Storage.save(CONFIG.STORAGE_KEYS.TASKS, tasks); this.render();
  }
};
...
// ============================================================
// SWIPE / DRAG to navigate calendar (all views)
// ============================================================
(function() {
  let sx = 0, sy = 0, active = false, moved = false;
  const MIN = 50;

  function inCal(target) { const c = document.getElementById('cal-container'); return c && c.contains(target); }
  function isBtn(target) { return !!target.closest('button, a, .ev-block, .tg-allday-ev, .month-mini-ev, .mini-month, input, select'); }

  document.addEventListener('touchstart', e => {
    if (!inCal(e.target)) return;
    sx = e.touches[0].clientX; sy = e.touches[0].clientY; active = true; moved = false;
  }, { passive: true });
  document.addEventListener('touchmove', () => { if (active) moved = true; }, { passive: true });
  document.addEventListener('touchend', e => {
    if (!active) return; active = false; if (!moved) return;
    const dx = e.changedTouches[0].clientX - sx, dy = e.changedTouches[0].clientY - sy;
    if (Math.abs(dx) < MIN || Math.abs(dy) > Math.abs(dx)) return;
    if (!CalSlider._animating) Calendar.navigate(dx < 0 ? 1 : -1);
  }, { passive: true });

  document.addEventListener('mousedown', e => {
    if (!inCal(e.target) || isBtn(e.target)) return;
    sx = e.clientX; sy = e.clientY; active = true; moved = false;
  });
  document.addEventListener('mousemove', e => { if (active && (Math.abs(e.clientX-sx) > 4 || Math.abs(e.clientY-sy) > 4)) moved = true; });
  document.addEventListener('mouseup', e => {
    if (!active) return; active = false; if (!moved) return;
    // Don't fire if the user was scrolling the time grid vertically
    if (e.target.closest('.tg-scroll')) return;
