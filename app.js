...
function applyTheme(mode, variant = load('darkVariant', 'classic')) {
  ...
  html.setAttribute('data-dark-variant', nextMode === 'dark' ? variant : '');
...
function toggleTheme() {
  const current = load('themeMode', 'light');
...
function setDarkVariant(variant) {
  ...
  const select = document.getElementById('dark-variant-select');
  if (select && select.value !== safeVariant) {
    select.value = safeVariant;
  }
}
...
function deleteDeadline(id) {
  ...
  const dl = deadlines.find(d => String(d.id) === String(id));
  deadlines = deadlines.filter(d => String(d.id) !== String(id));
  save('deadlines', deadlines);
  renderDeadlines();

  // If this deadline is backed by a Google Calendar event, delete it there too
  if (dl && dl.gcalEventId && isTokenValid()) {
    fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events/${dl.gcalEventId}`,
...
function confirmDeadline() {
  ...
  const id = Date.now();
  const deadline = { id, name, course, date };
  ...
  // Also add this as an all-day event in Google Calendar if connected
  if (isTokenValid()) {
    ...
    fetch(
      ...
      .then(data => {
        if (data && !data.error) {
          deadlines = deadlines.map(d => String(d.id) === String(id)
            ? { ...d, gcalEventId: data.id }
            : d
          );
          save('deadlines', deadlines);
