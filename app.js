...
function deleteDeadline(id) {
  ...
  // Check if it's a manual deadline
  ...
  // Check if it's a gcal auto-detected deadline
  ...
  if (gcal && gcal.gcalEventId && isTokenValid()) {
    fetch(
      ...
      .then(() => fetchDeadlinesFromCalendar())
...
function confirmDeadline() {
  ...
  if (isTokenValid()) {
    ...
    fetch(
      ...
      .then(data => {
        if (data && !data.error) {
          // Store the gcal event id on the local deadline so delete works
          deadlines = deadlines.map(d => d.id === id ? { ...d, gcalEventId: data.id } : d);
          save('deadlines', deadlines);
          // Refresh gcal auto-deadlines in background too
          fetchDeadlinesFromCalendar();
...
// Cache list IDs so we don't have to look them up every time
...
async function ensureGTaskLists() {
  try {
    ...
    if (!gTaskListIds.todo) {
      ...
      if (!created.error) gTaskListIds.todo = created.id;
    }
    if (!gTaskListIds.assignment) {
      ...
      const created = await res.json();
      if (!created.error) gTaskListIds.assignment = created.id;
    }
...
async function pushToGoogleTasks(text, type, localId) {
  try {
    ...
    const created = await res.json();
    if (!created.error) {
      // Store gtaskId on the local task so we can delete it later
      tasks = tasks.map(t => String(t.id) === String(localId) ? { ...t, gtaskId: created.id, gtaskListId: listId } : t);
      save('tasks', tasks);
    }
...
async function fetchGoogleTasks() {
  try {
    await ensureGTaskLists();
    const existingGtaskIds = new Set(tasks.filter(t => t.gtaskId).map(t => t.gtaskId));
    let added = 0;
    for (const [type, listId] of [['todo', gTaskListIds.todo], ['assignment', gTaskListIds.assignment]]) {
      if (!listId) continue;
      const data = await gFetch(
        `https://tasks.googleapis.com/tasks/v1/lists/${listId}/tasks?showCompleted=false&maxResults=50`
      );
      (data.items || []).forEach(t => {
        if (!t.title || existingGtaskIds.has(t.id)) return;
        tasks.push({ id: 'gt_' + t.id, gtaskId: t.id, gtaskListId: listId, text: t.title, type, done: false, source: 'gtasks' });
        existingGtaskIds.add(t.id);
        added++;
      });
    }
...
async function addTask() {
  ...
  // Push to the matching Google Tasks list if connected
  if (isTokenValid()) {
...
async function deleteTask(id) {
  ...
  // Also delete from Google Tasks if it was pushed there
