 ...
 * Features: Horizontal scroll calendar (Apple-like), urgent tasks, responsive layout
 */
...
// ============================================================
// CALENDAR MODULE (with horizontal scroll)
// ============================================================

const Calendar = {
  render() {
    const view = AppState.get().calView;
    if (view === 'day') this.renderHorizontalDays();
    else if (view === 'week') this.renderHorizontalWeeks();
  ...
  async fetchEvents() {
    ...
    // For horizontal scroll, we need a larger range
    let actualStart = start, actualEnd = end;
    if (state.calView === 'day') {
      actualStart = new Date(start);
      actualStart.setDate(start.getDate() - 10);
      actualEnd = new Date(start);
      actualEnd.setDate(start.getDate() + 10);
    } else if (state.calView === 'week') {
      actualStart = new Date(start);
      actualStart.setDate(start.getDate() - 21);
      actualEnd = new Date(end);
      actualEnd.setDate(end.getDate() + 21);
    }
    
    const { data, error } = await safeFetch(GoogleAPI.fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events` +
      `?timeMin=${actualStart.toISOString()}&timeMax=${actualEnd.toISOString()}` +
  ...
  // Horizontal scroll for day view (7+ days)
  renderHorizontalDays() {
    const state = AppState.get();
    const centerDate = new Date(state.calDate);
    const days = [];
    for (let i = -7; i <= 7; i++) {
    ...
    const label = `${CONFIG.MONTHS_SH[centerDate.getMonth()]} ${centerDate.getDate()}, ${centerDate.getFullYear()}`;
    document.getElementById('cal-nav-label').textContent = label;
    
    const container = document.getElementById('cal-container');
    container.innerHTML = `
      <div class="horizontal-scroll-container" id="hs-container">
        <div class="scroll-pane" id="scroll-pane">
          ${days.map(d => `<div class="scroll-item" data-date="${d.toISOString()}">${this._buildTimeGrid([d])}</div>`).join('')}
        </div>
      </div>
    `;
    // Scroll to center (the 8th item)
    const scrollDiv = document.getElementById('hs-container');
    if (scrollDiv) {
      setTimeout(() => {
        const items = document.querySelectorAll('.scroll-item');
        if (items[7]) items[7].scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'start' });
      }, 50);
      // Add scroll end listener to update date
      let scrollTimeout;
        ...
        scrollTimeout = setTimeout(() => {
          ...
          if (closest) {
            ...
            if (dateStr) {
              ...
              if (newDate.toDateString() !== state.calDate.toDateString()) {
                AppState.set({ calDate: newDate });
                document.getElementById('cal-nav-label').textContent = `${CONFIG.MONTHS_SH[newDate.getMonth()]} ${newDate.getDate()}, ${newDate.getFullYear()}`;
                // Optionally fetch more events if needed
                if (GoogleAPI.isTokenValid()) this.fetchEvents();
              }
            }
          }
  ...
  // Horizontal scroll for week view (3 weeks)
  renderHorizontalWeeks() {
    const state = AppState.get();
    const centerWeekStart = new Date(state.calDate);
    centerWeekStart.setDate(state.calDate.getDate() - state.calDate.getDay());
    centerWeekStart.setHours(0,0,0,0);
    const weeks = [];
    for (let i = -2; i <= 2; i++) {
      const weekStart = new Date(centerWeekStart);
      weekStart.setDate(centerWeekStart.getDate() + i * 7);
      const weekEnd = new Date(weekStart);
      weekEnd.setDate(weekStart.getDate() + 6);
      weeks.push({ start: weekStart, end: weekEnd });
    }
    const label = `${CONFIG.MONTHS_SH[centerWeekStart.getMonth()]} ${centerWeekStart.getDate()} – ${CONFIG.MONTHS_SH[weekEnd.getMonth()]} ${weekEnd.getDate()}, ${centerWeekStart.getFullYear()}`;
    document.getElementById('cal-nav-label').textContent = label;
      ...
      <div class="horizontal-scroll-container" id="hs-container">
        <div class="scroll-pane" id="scroll-pane">
          ${weeks.map((week, idx) => {
            ...
            return `<div class="scroll-item" data-weekstart="${week.start.toISOString()}">${this._buildTimeGrid(days)}</div>`;
          }).join('')}
        </div>
      </div>
    `;
    // Scroll to center week (index 2)
    const scrollDiv = document.getElementById('hs-container');
    if (scrollDiv) {
      setTimeout(() => {
        const items = document.querySelectorAll('.scroll-item');
        if (items[2]) items[2].scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'start' });
      }, 50);
      let scrollTimeout;
      scrollDiv.addEventListener('scroll', () => {
        clearTimeout(scrollTimeout);
        scrollTimeout = setTimeout(() => {
          ...
          if (closest) {
            const weekStartStr = closest.getAttribute('data-weekstart');
            if (weekStartStr) {
              const newWeekStart = new Date(weekStartStr);
              if (newWeekStart.toDateString() !== centerWeekStart.toDateString()) {
                AppState.set({ calDate: newWeekStart });
                // update label
                const newEnd = new Date(newWeekStart);
                newEnd.setDate(newWeekStart.getDate() + 6);
                document.getElementById('cal-nav-label').textContent = `${CONFIG.MONTHS_SH[newWeekStart.getMonth()]} ${newWeekStart.getDate()} – ${CONFIG.MONTHS_SH[newEnd.getMonth()]} ${newEnd.getDate()}, ${newWeekStart.getFullYear()}`;
                if (GoogleAPI.isTokenValid()) this.fetchEvents();
              }
            }
          }
...
const Tasks = {
  render() {
    ...
    // Sort urgent to top, then by done (false first), then by id
    ...
    list.innerHTML = filtered.map(t => `<div class="task-item${t.done ? ' done' : ''}${t.urgent ? ' urgent' : ''}">
      ...
      <button class="urgent-toggle ${t.urgent ? 'active' : ''}" onclick="window.toggleUrgent('${t.id}')" title="Mark urgent">!</button>
      <button class="task-delete" onclick="window.deleteTask('${t.id}')">✕</button>
    </div>`).join('');
  },
  ...
  async add() {
    ...
    const urgent = document.getElementById('task-urgent')?.checked || false;
    const id = Date.now();
  ...
  async toggleUrgent(id) {
    ...
    Storage.save(CONFIG.STORAGE_KEYS.TASKS, tasks);
    this.render();
    // urgent flag is local only, no Google sync
  },
  ...
  async fetchFromGoogle() {
    ...
    incoming.forEach(gt => {
      const byId = localByGtaskId[gt.gtaskId];
      if (byId) {
        tasks.push({ ...byId, done: gt.done, text: gt.text });
      } else {
        const key = gt.text + '||' + gt.type;
        const byText = localByTextType[key];
        if (byText) {
          tasks = tasks.filter(t => t.id !== byText.id);
          tasks.push({ ...byText, gtaskId: gt.gtaskId, gtaskListId: gt.gtaskListId, done: gt.done });
        } else {
          tasks.push({ id: 'gt_' + gt.gtaskId, gtaskId: gt.gtaskId, gtaskListId: gt.gtaskListId, text: gt.text, type: gt.type, done: gt.done, urgent: false, source: 'gtasks' });
        }
      }
    });
...
// Re-sync tasks when tab becomes visible again
