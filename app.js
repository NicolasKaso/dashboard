...
// ============================================================
// CALENDAR NAVIGATION — simple slide animation + horizontal wheel
// No 3-panel tricks. Just animate the container out, swap content, animate in.
// ============================================================
...
const CalNav = {
  ...
  // Render content into container with no animation (initial load or view switch)
  render(container, html) {
    container.innerHTML = html;
  },

  // Animate out → swap content → animate in
  // dir: 1 = forward (slide from right), -1 = back (slide from left)
  slide(container, html, dir) {
    if (this._animating) return;

    // Save vertical scroll before swapping
    const scrollEl = container.querySelector('.tg-scroll');
    if (scrollEl) this._savedScroll = scrollEl.scrollTop;

    this._animating = true;
    const outClass = dir === 1 ? 'cal-slide-out-left' : 'cal-slide-out-right';
    const inClass  = dir === 1 ? 'cal-slide-in-right' : 'cal-slide-in-left';

    // Step 1: add out-animation class
    container.classList.add(outClass);

    // Step 2: after out-animation completes, swap content and animate in
    // Use animationend (not transitionend — we're using @keyframes, not transitions)
    let swapped = false;
    const doSwap = () => {
      ...
      setTimeout(cleanUp, 300); // safety
    };

    container.addEventListener('animationend', doSwap, { once: true });
    setTimeout(doSwap, 300); // safety if animationend doesn't fire
  }
};
...
(function() {
  ...
  document.getElementById('cal-container')?.addEventListener('wheel', function(e) {
    ...
    e.preventDefault();
    if (wheelCooldown || CalNav._animating) return;
    wheelCooldown = true;
