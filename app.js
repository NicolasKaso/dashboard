...
const CalSlider = {
  ...
  slide(container, buildFn, newCenterDate, view, dir) {
    ...
    // Step 1: animate the EXISTING content sliding out
    // dir 1 (forward) = track moves left (-200%), dir -1 (back) = track moves right (0%)
    const to = dir === 1 ? -200 : 0;

    // Ensure track starts at resting position (-100%)
    ...
    const done = () => {
      // Step 2: rebuild panels with new dates (invisible — track is off-screen)
      ...
      // Step 3: snap back to center (no transition)
      track.style.transition = 'none';
      track.style.transform = 'translateX(-100%)';
    ...
    setTimeout(() => { if (this._animating) done(); }, 400);
  },
...
const Tasks = {
  ...
  async add() {
    ...
    const urgent = urgentBtn ? urgentBtn.classList.contains('active') : false;
    const id = Date.now();
    ...
    input.value = '';
    // Reset urgent button after adding
