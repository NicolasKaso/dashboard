 ...
 * Added: Horizontal scroll for day/week, Urgent tasks, responsive task input
 */
...
// ============================================================
// CALENDAR MODULE (with horizontal scroll for day/week)
// ============================================================
...
const Calendar = {
  ...
  async fetchEvents() {
    ...
    // For horizontal scroll we need a larger range to preload adjacent days/weeks
    let actualStart = start, actualEnd = end;
    if (state.calView === 'day') {
      actualStart = new Date(start);
      actualStart.setDate(start.getDate() - 14);
      actualEnd = new Date(start);
      actualEnd.setDate(start.getDate() + 14);
    } else if (state.calView === 'week') {
      actualStart = new Date(start);
  ...
  // Day view with horizontal scroll (15 days)
  renderDay() {
    const state = AppState.get();
    const centerDate = new Date(state.calDate);
    // Build 15 days: 7 left, center, 7 right
    const days = [];
    for (let i = -7; i <= 7; i++) {
    ...
    const label = `${CONFIG.DAYS_LONG[centerDate.getDay()]}, ${CONFIG.MONTHS_SH[centerDate.getMonth()]} ${centerDate.getDate()}`;
    document.getElementById('cal-nav-label').textContent = label;
    
    const container = document.getElementById('cal-container');
    // Build horizontal scroll wrapper
    let html = `<div class="horizontal-scroll-container" id="hs-container"><div class="scroll-pane">`;
    days.forEach(d => {
      html += `<div class="scroll-item" data-date="${d.toISOString()}">${this._buildTimeGrid([d])}</div>`;
    });
    html += `</div></div>`;
    container.innerHTML = html;
    
    this._setupHorizontalScroll('day');
    this._scrollToHour(8);
  },
  
  // Week view with horizontal scroll (5 weeks)
  renderWeek() {
    const state = AppState.get();
    ...
    const label = `${CONFIG.MONTHS_SH[centerWeekStart.getMonth()]} ${centerWeekStart.getDate()} – ${CONFIG.MONTHS_SH[weeks[2].end.getMonth()]} ${weeks[2].end.getDate()}, ${centerWeekStart.getFullYear()}`;
    document.getElementById('cal-nav-label').textContent = label;
    ...
    weeks.forEach((week, idx) => {
      ...
      html += `<div class="scroll-item" data-weekstart="${week.start.toISOString()}">${this._buildTimeGrid(days)}</div>`;
    });
    html += `</div></div>`;
    container.innerHTML = html;
    
    this._setupHorizontalScroll('week');
    this._scrollToHour(8);
  },
  
  _setupHorizontalScroll(mode) {
    const scrollDiv = document.getElementById('hs-container');
    if (!scrollDiv) return;
    // Scroll to center item (index depends on number of items)
    setTimeout(() => {
      const items = document.querySelectorAll('.scroll-item');
      const centerIndex = Math.floor(items.length / 2);
      if (items[centerIndex]) {
        items[centerIndex].scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'start' });
      }
    }, 50);
          ...
          if (mode === 'day') {
            ...
            if (dateStr) {
              ...
              if (newDate.toDateString() !== state.calDate.toDateString()) {
                AppState.set({ calDate: newDate });
                const label = `${CONFIG.DAYS_LONG[newDate.getDay()]}, ${CONFIG.MONTHS_SH[newDate.getMonth()]} ${newDate.getDate()}`;
                document.getElementById('cal-nav-label').textContent = label;
                if (GoogleAPI.isTokenValid()) this.fetchEvents();
              }
            }
            ...
            if (weekStartStr) {
              ...
              const state = AppState.get();
              if (newWeekStart.toDateString() !== state.calDate.toDateString()) {
                AppState.set({ calDate: newWeekStart });
                const weekEnd = new Date(newWeekStart);
                weekEnd.setDate(newWeekStart.getDate() + 6);
                const label = `${CONFIG.MONTHS_SH[newWeekStart.getMonth()]} ${newWeekStart.getDate()} – ${CONFIG.MONTHS_SH[weekEnd.getMonth()]} ${weekEnd.getDate()}, ${newWeekStart.getFullYear()}`;
                document.getElementById('cal-nav-label').textContent = label;
                if (GoogleAPI.isTokenValid()) this.fetchEvents();
              }
            }
...
// ============================================================
// TASKS MODULE (with urgent flag and sorting)
// ============================================================
...
const Tasks = {
  render() {
    ...
    // Sort: urgent first, then undone, then by id
  ...
  async toggleUrgent(id) {
    ...
    Storage.save(CONFIG.STORAGE_KEYS.TASKS, tasks);
    this.render();
    // urgent is local only, no Google sync
  },
...
// Re-sync tasks when tab becomes visible again (switching between devices/tabs)
...
// Periodic sync every 60 seconds to keep tasks current across devices
setInterval(() => {
