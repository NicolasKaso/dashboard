...
// Returns { todo: id, assignment: id } — always fetches fresh, creates if missing
async function resolveGTaskLists() {
  const data = await gFetch('https://tasks.googleapis.com/tasks/v1/users/@me/lists?maxResults=20');
  const lists = data.items || [];
  const ids = { todo: null, assignment: null };
  ...
  if (!ids.todo) {
    ...
    if (created.error) throw new Error('Could not create To-do list: ' + created.error.message);
    ids.todo = created.id;
  }
  if (!ids.assignment) {
    const res  = await fetch('https://tasks.googleapis.com/tasks/v1/users/@me/lists', {
    ...
    const created = await res.json();
    if (created.error) throw new Error('Could not create Assignments list: ' + created.error.message);
...
async function pushToGoogleTasks(text, type, localId) {
  try {
    const ids    = await resolveGTaskLists();
    const listId = type === 'assignment' ? ids.assignment : ids.todo;
    if (!listId) { console.error('pushToGoogleTasks: no listId for type', type); return; }
    ...
    const created = await res.json();
    if (created.error) {
      console.error('pushToGoogleTasks API error:', created.error.message);
      return;
    }
    tasks = tasks.map(t => String(t.id) === String(localId)
      ? { ...t, gtaskId: created.id, gtaskListId: listId } : t);
    save('tasks', tasks);
  } catch (err) { console.error('pushToGoogleTasks failed:', err); }
}
