  ...
  async fetch(url) {
    ...
    if (data.error) {
      if (data.error.code === 401) {
        AppState.set({ accessToken: null, tokenExpiry: 0, gTaskListIds: { todo: null, assignment: null, urgent: null } });
...
const Tasks = {
  ...
  async toggleUrgent(id) {
    ...
    let tasks = AppState.get().tasks.map(t => String(t.id) === String(id) ? { ...t, urgent: toUrgent } : t);
    AppState.set({ tasks }); Storage.save(CONFIG.STORAGE_KEYS.TASKS, tasks); this.render();
    // Move in Google Tasks if connected
    if (GoogleAPI.isTokenValid()) {
      const result = await this.moveInGoogle(task, toUrgent);
      if (result) {
        tasks = AppState.get().tasks.map(t => String(t.id) === String(id) ? { ...t, gtaskId: result.gtaskId, gtaskListId: result.gtaskListId } : t);
        AppState.set({ tasks }); Storage.save(CONFIG.STORAGE_KEYS.TASKS, tasks);
      }
    }
  },
  ...
  async resolveTaskLists() {
    // Use cached IDs if all three lists are resolved
    ...
    let ids = { todo: null, assignment: null, urgent: null };
  ...
  // Move a task between Google Task lists (used when toggling urgent on existing tasks)
  async moveInGoogle(task, toUrgent) {
    if (!GoogleAPI.isTokenValid()) return null;
    try {
      const ids = await this.resolveTaskLists();
      const newListId = toUrgent ? ids.urgent : (task.type === 'assignment' ? ids.assignment : ids.todo);
      // Create in new list
  ...
  async fetchFromGoogle() {
    ...
    try {
      ...
      // Build map of all remote tasks keyed by gtaskId
      const remote = {};
      ...
      // Index local tasks by gtaskId to preserve any local-only fields
      const localByGtaskId = {};
      AppState.get().tasks.forEach(t => { if (t.gtaskId) localByGtaskId[t.gtaskId] = t; });

      // Remote is source of truth — preserve local id for continuity
