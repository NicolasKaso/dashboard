...
const Tasks = {
  ...
  async fetchFromGoogle() {
    ...
    // Also index local tasks by text+type so we can link un-pushed local tasks to their remote counterpart
    ...
    // Start from scratch: keep only local-only tasks that have no gtaskId
    // (they haven't been pushed yet — keep them until they get linked or we push them)
    let tasks = AppState.get().tasks.filter(t => !t.gtaskId);

    incoming.forEach(gt => {
      const byId = localByGtaskId[gt.gtaskId];
      if (byId) {
        // Already linked — update done status from remote (remote is source of truth)
        tasks.push({ ...byId, done: gt.done, text: gt.text });
      } else {
        // Check if a local un-pushed task matches by text+type — link it
...
// ============================================================
// CALENDAR SWIPE / DRAG TO NAVIGATE
// ============================================================
(function initCalSwipe() {
  let startX = 0, startY = 0, isDragging = false, moved = false;
  const THRESHOLD = 50; // px needed to trigger navigation
  const VERTICAL_LOCK = 10; // if moved more than this vertically first, ignore

  function getCalEl() { return document.getElementById('cal-container'); }

  // Touch support (mobile/trackpad swipe)
  ...
  document.addEventListener('touchend', (e) => {
    ...
    Calendar.navigate(dx < 0 ? 1 : -1);
  }, { passive: true });

  // Mouse drag support (desktop / external monitor)
  document.addEventListener('mousedown', (e) => {
    const el = getCalEl();
    if (!el || !el.contains(e.target)) return;
    // Only start drag on the grid background, not on event blocks or buttons
    if (e.target.closest('.ev-block, button, a, .tg-allday-ev, .month-mini-ev, .mini-month')) return;
    startX = e.clientX;
    startY = e.clientY;
    isDragging = true;
    moved = false;
  ...
  document.addEventListener('mouseup', (e) => {
    ...
    if (Math.abs(dy) > Math.abs(dx) + VERTICAL_LOCK) return; // more vertical than horizontal
    if (Math.abs(dx) < THRESHOLD) return;
    Calendar.navigate(dx < 0 ? 1 : -1);
  });
})();
